...
impl IPv6PortSpaceTree {
    ...
    pub fn clear_and_print_records(aliased_prefixes:&AHashSet<u64>, records:Vec<(u128, u16, u32)>, output:&mut Box<dyn OutputMethod>, region_len:usize) -> (Vec<u64>, u64) {
        // 生成 区域编码记录器
    ...
    pub fn print_records(records:Vec<(u128, u16, u32)>, output:&mut Box<dyn OutputMethod>, region_len:usize) -> (Vec<u64>, u64) {
        // 生成 区域编码记录器
        let mut region_recorder:Vec<u64> = vec![0u64; region_len];
        region_recorder.shrink_to_fit();
    ...
    fn gen_targets(prefix_64:u128, port:u16, code:Vec<u8>, rng:&mut StdRng, targets:&mut Vec<(Vec<u8>, u16, u128)>){
