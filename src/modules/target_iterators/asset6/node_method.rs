...
impl IPv6PortSpaceTreeNode {
    ...
    fn expand_dim(&mut self, expand_mask:U144) {
        ...
        let modes_set:AHashSet<U144> = self.modes.iter()
    ...
    fn gen_addrs_by_all_modes(&self, dim_size:u128) -> AHashSet<U144> {
        // 生成的全部地址
        ...
        for left_move_len in gen_move_len {
            ...
            let mut cur_addrs:AHashSet<U144> = AHashSet::new();
            for mode in generated_addrs.iter() {

                for dim_size in 1..dim_size {
                    let tar_addr = (*mode) | (U144::from(dim_size) << left_move_len);
    ...
    /// 根据 本区域的指定预算, 生成 目标地址
    #[allow(unused_assignments)]
    pub fn gen_tars(&mut self, region_budget:usize, tree:&IPv6PortSpaceTree) -> AHashSet<U144> {
        ...
        let mut expanded = false;
        {
            ...
            while space_needed > self.space_size {
                ...
                // 计算 扩展掩码
                let expand_mask = !(tree.split_mask_u144 << cur_split);
        ...
        let mut selected_tars:AHashSet<U144> = AHashSet::with_capacity(region_budget);
        {
            // 生成随机种子
            let mut rng = rand::thread_rng();

            // 随机选择 目标地址, 并生成 目标地址列表
            selected_tars = self.no_used_generated_address
            ...
            self.no_used_generated_address = self.no_used_generated_address.difference(&selected_tars).cloned().collect();
            // 向 已使用的地址 中插入 当次使用地址
            self.used_addrs.extend(&selected_tars);
        }
