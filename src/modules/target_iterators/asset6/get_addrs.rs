...
impl IPv6PortSpaceTree {
    ...
    /// 注意: 生成目标 (区域编码， 端口， 目标地址)
    pub fn gen_addrs(&mut self, cur_budget:u64, aliased_prefixes:&AHashSet<u64>) -> Vec<(Vec<u8>, u16, u128)> {
        ...
        // 目标队列
        let mut targets:Vec<(Vec<u8>, u16, u128)> = Vec::with_capacity(cur_budget as usize);

        // 计算 前max_node_num个 奖励的总和
        let sum_reward:f64 = self.all_reward.iter().take(cur_extra_region_num).sum();
        // 16位1
        let u16_max = U144::from(u16::MAX);
        let id2port = &self.id2port;

        // 当前端口扫描标志字段
        let port_scan_flag = self.port_scan_flag;

        for i in 0..cur_extra_region_num {
            let i_u32 = i as u32;
            let mut code = i_u32.to_be_bytes();
            // 将 大端编码的第一个字节附加标志
            code[0] = port_scan_flag;
            let code = code.to_vec();

            let mut node = (&self.region_queue[i]).borrow_mut();

            // 获得生成的 地址集合
            let cur_tars_set:AHashSet<U144>;
            {
                // 计算当前节点的预算值
                let region_budget = ((self.all_reward[i] / sum_reward) * (cur_budget as f64)) as usize;

                // 在当前节点生成 目标地址集合
                let region_gen_addrs = node.gen_tars(region_budget, &self);

                // 当前目标集合 = 生成的目标集合 - 已使用的总集合
                cur_tars_set = region_gen_addrs.difference(&self.used_addrs).cloned().collect();
            }

            // 注意: cur_extra_region_num 不能超过 u16::MAX

            // 将 (区域编码, 本区域地址) 写入目标队列
            for &tar in &cur_tars_set {
                
                // 注意: 由生成目标解析出 地址 和 端口
                let tar_ip = (tar >> 16).as_u128();
                
                // 计算当前地址的/64前缀
                let tar_prefix = (tar_ip >> 64) as u64;
                // 如果是别名前缀
                if aliased_prefixes.contains(&tar_prefix) { continue }
                
                let tar_id = (tar & u16_max).as_u32() as u16;
                let tar_port = *id2port.get(&tar_id).unwrap();
                
                targets.push((code.clone(), tar_port, tar_ip));
            }

            // 向 已使用的地址 集合中添加 当前集合
            self.used_addrs.extend(cur_tars_set);
        }
            ...
            if cur_node.no_used_generated_address.is_empty() && cur_node.split_stack.is_empty() {
                ...
                {// 将 该节点持有的信息转交父节点
                    ...
                    {// 将 子节点中存在 而 父节点中不存在的模式 插入父节点的模式
                        let mut new_modes: Vec<U144> = Vec::new();
