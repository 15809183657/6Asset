...
use std::rc::Rc;
use ahash::AHashSet;
use crate::modules::target_iterators::asset6::U144;

pub struct IPv6PortSpaceTreeNode {
    
    //  标识
    pub id:u32,

    // 父节点指针
    pub parent:Option<Rc<RefCell<IPv6PortSpaceTreeNode>>>,

    // 节点层级
    pub level:u8,
    
    // 该节点的子节点指针
    pub children:Vec<Rc<RefCell<IPv6PortSpaceTreeNode>>>,


    // 该聚类区域的 (种子地址,端口)
    // 初始化空间树 以及 第一次生成目标时使用, 后续将被置空
    pub seed_addrs_list:Vec<U144>,
    ...
    // 生成但未使用的目标
    // 注意: 使用后的目标应从该列表删除
    pub no_used_generated_address:AHashSet<U144>,
    ...
    // 该区域已经使用的目标
    // 注意: 第一次生成的目标必然包含全部种子目标, 后续也是, used_addrs应始终包含 种子目标
...
impl IPv6PortSpaceTreeNode {
    pub fn new(id:u32, seed_addrs_list:Vec<U144>,
               split_stack:Vec<u8>, split_move_len:u8, act_val_num:Vec<(usize, u64)>,
               parent:Option<Rc<RefCell<IPv6PortSpaceTreeNode>>>, level:u8) -> Self {
