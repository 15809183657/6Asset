...
impl IPv6PortSpaceTree {
    ...
    pub fn init_density_tree(&mut self){
        ...
        // 生成 由 id到端口的转换
        self.id2port = Self::get_mapping(&get_current_path(&SYS.get_info("conf", "asset6_id2port")));

        let mut seeds:Vec<U144>;
        {
            // 从 文件 中读取种子地址
        ...
        let root = IPv6PortSpaceTreeNode::new(self.id_num, seeds, stack, split_move_len, act_val_num, None, 0);
        self.id_num += 1;
    ...
    /// 递归分割形成密度空间树
    pub fn density_split_tree(&mut self, node:Rc<RefCell<IPv6PortSpaceTreeNode>>){

        let mut cur_node: RefMut<IPv6PortSpaceTreeNode> = node.borrow_mut();
        ...
        let mut key_ips:Vec<Vec<U144>>;
        {
            ...
            let split_mask = self.split_mask_u144;
            for addr in cur_node.seed_addrs_list.iter() {
                // 对于 整个地址聚类块 中的 每个地址

                // 计算当前地址在 分割点 上的值
                let val = ((addr >> split_move_len) & split_mask).as_usize();
        ...
        for ips in key_ips {
            ...
            let child = Rc::new(RefCell::new(IPv6PortSpaceTreeNode::new(self.id_num, ips, stack, split_move_len, act_val_num, Some(node.clone()), cur_node.level + 1)));
            self.id_num += 1;
    ...
    fn get_split_stack_density(&self, addrs:&Vec<U144>, move_len:&Vec<u8>) -> (u8, Vec<(usize, u64)>, Vec<u8>) {

        // 注意: 该栈的顺序为  低熵 -> 高熵
        // 注意: 当熵值相等时为    地址结构较左 ->  地址结构较右

        // 获取 地址结构段 的统计信息
        let stat = Self::get_stat(addrs, move_len, self.dim_size, self.split_mask_u144);
        ...
        // 对每个地址结构段进行熵值计算
        let port_entropy_mul = self.port_entropy_mul;
        for (index, local_stat) in stat.iter().enumerate() {
            ...
            let cur_entropy = {
                // 取出 对应的右移距离
                let cur_move_len = move_len[index];
                
                if cur_move_len < 16 {
                    // 警告: 当右移距离小于16时，意味着处理的是端口部分
                    port_entropy_mul * Self::get_entropy(pro_list, dim_f64)
                } else {
                    Self::get_entropy(pro_list, dim_f64)
                }
            };
