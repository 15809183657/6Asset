    ...
    fn execute(&self) {
        ...
        loop {
            ...
            let targets: Vec<(Vec<u8>, u16, u128)>;
            {
                ...
                loop {
                    let cur_tars = space_tree.gen_addrs(cur_budget, &aliased_prefixes);
            ...
            let receiver_res;
            {
                ...
                receiver_res = thread::spawn(move || {
                    PcapReceiver::space_tree_run_v6_port_vec(0, base_conf, receiver_conf, 
            ...
            // 等待接收线程按照预定时间关闭
            let cur_records = if let Ok((records, aliased_prefixes_)) = receiver_res.join() {
            ...
            if !cur_records.is_empty() {
                ...
                if self.aliased_prefixes_check {
                    ...
                    if let Ok(aliased_check_res) = aliased_check_receiver_res.join() {
                        ...
                        let (region_recorder, cur_act_num) = IPv6PortSpaceTree::clear_and_print_records(&cur_aliased_prefixes, cur_records, &mut out_mod, space_tree.cur_extra_region_num);
                        // 将当前轮次的别名前缀向总别名前缀集合并
                    ...
                    // 如果不开启别名检查
                    let (region_recorder, cur_act_num) = IPv6PortSpaceTree::print_records(cur_records, &mut out_mod, space_tree.cur_extra_region_num);
                    info!("{} {}", SYS.get_info("info", "cur_active_num"),cur_act_num);
                    total_active_num += cur_act_num;
                    space_tree.update_tree(region_recorder);
                }
