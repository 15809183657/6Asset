...
impl PcapReceiver {
    pub fn space_tree_run_v6_port_vec(interface_index:usize,
                                 base_conf:Arc<BaseConf>, receiver_conf:Arc<ReceiverBaseConf>,
                                 // 探测模块用来验证信息, 处理数据包
                                 probe_mod:Arc<CodeProbeModV6>, addrs_len:usize, sports:Vec<u16>, aliased_prefixes:AHashSet<u64>, scan_flag:u8, region_len:u32,
                                 recv_ready_sender:Sender<bool>, recv_close_time_receiver:Receiver<i64>) -> (Vec<(u128, u16, u32)>, AHashSet<u64>) {
        ...
        // 初始化 探测模块
        let probe = CodeProbeModV6::init(probe_mod, sports);

        // 复制加密器
        let aes_rand = base_conf.aes_rand.clone();

        // 创建 哈希查重器
        let mut hash_set:AHashSet<(u128, u16, u32)> = AHashSet::with_capacity(addrs_len);
            ...
            match packet {
                ...
                Ok(packet) => {
                    ...
                    if ip_ver == 6 {
                        ...
                        Self::handle_packet_v6_port_vec(net_layer_data, &aes_rand, &probe, &mut hash_set, &aliased_prefixes, scan_flag, region_len);
                    }
        ...
        let mut records:Vec<(u128, u16, u32)> = hash_set.into_iter().collect();

        // 将所有记录随机化
        let rng:&mut StdRng = &mut StdRng::from_entropy();
        records.shuffle(rng);

        (records, aliased_prefixes)
    }
}